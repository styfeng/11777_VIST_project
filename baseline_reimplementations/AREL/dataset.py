...
class VISTDataset(Dataset):
    def __init__(self, opt):
        ...
        filedir = os.path.dirname(opt.story_line_json)
        stories_subset = {}
        stories_subset['train'] = json.load(open(os.path.join(filedir,'stories_train.json')))
        stories_subset['val'] = json.load(open(os.path.join(filedir,'stories_val.json')))
        stories_subset['test'] = json.load(open(os.path.join(filedir,'stories_test.json')))
        print('DataLoader filtering stories from subset json files: ', filedir+'/stories_{train/val/test}.json')
        
        self.story_ids = {'train': [], 'val': [], 'test': []}
        self.description_ids = {'train': [], 'val': [], 'test': []}
        self.story_ids['train'] = [key for key in self.story_line['train'].keys() if key in stories_subset['train']]
        self.story_ids['val']   = [key for key in self.story_line['val'].keys() if key in stories_subset['val']]
        self.story_ids['test']  = [key for key in self.story_line['test'].keys() if key in stories_subset['test']]
        ...
        for split in ['val', 'test']:
            ...
            for story_id,story in self.story_line[split].items():
                if story_id not in stories_subset[split]: continue
                if story['album_id'] not in reference:
                    reference[story['album_id']] = [story['origin_text']]
                    reference_captions[story['album_id']] = [stories_subset[split][story_id]['captions']]
                else:
                    reference[story['album_id']].append(story['origin_text'])
                    reference_captions[story['album_id']].append(stories_subset[split][story_id]['captions'])
            with open(os.path.join(ref_dir, '{}_reference.json'.format(split)), 'w') as f:
                json.dump(reference, f)
            with open(os.path.join(ref_dir, '{}_reference_captions.json'.format(split)), 'w') as f:
                json.dump(reference_captions, f)
